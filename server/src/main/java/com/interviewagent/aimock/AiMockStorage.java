...
@Service
public class AiMockStorage {
    private final HttpClient client;
    private final String url, bucket, importBucket, key;
    public AiMockStorage(@Value("${app.supabase.storage-url}") String url, @Value("${app.ai-mock-audio.bucket:ai-mock-audio}") String bucket, @Value("${app.interview-import-audio.bucket:interview-import-audio}") String importBucket, @Value("${SUPABASE_STORAGE_SERVICE_KEY:}") String key) { this.url = url.replaceAll("/+$", ""); this.bucket = bucket; this.importBucket = importBucket; this.key = key; this.client = httpClient(); }
